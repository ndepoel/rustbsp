...
pub fn load_texture_nomipmap(queue: Arc<Queue>, img: RgbaImage) -> Result<(Arc<dyn ImageViewAbstract + Send + Sync>, CommandBufferExecFuture<NowFuture, AutoCommandBuffer>), ImageCreationError>
{
...
pub fn load_texture_mipmapped(queue: Arc<Queue>, img: RgbaImage) -> Result<(Arc<dyn ImageViewAbstract + Send + Sync>, CommandBufferExecFuture<NowFuture, AutoCommandBuffer>), ImageCreationError>
{
    let (w, h) = img.dimensions();
